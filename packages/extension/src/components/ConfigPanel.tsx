...
export function ConfigPanel({ config, onSave, onClose }: ConfigPanelProps) {
	...
	const [provider, setProvider] = useState<ExtConfig['provider']>(config?.provider ?? 'openai')
	const [selectedPresetId, setSelectedPresetId] = useState<ProviderPresetId>(
		config ? detectProviderPresetId(config) : 'custom'
	)
	...
	if (prevConfig !== config) {
		...
		setSelectedPresetId(config ? detectProviderPresetId(config) : 'custom')
	...
	const handlePresetChange = (presetId: ProviderPresetId) => {
		...
		setDisableNamedToolChoice(nextConfig.disableNamedToolChoice ?? false)
	}

	const markCustom = () => setSelectedPresetId('custom')
	...
	return (
		<div className="flex flex-col gap-4 p-4 relative">
			...
			>
				Manage Super Page Agent Hub
				<ExternalLink className="size-3" />
			</a>

			<div className="flex flex-col gap-1.5">
				<label htmlFor="provider-preset" className="text-xs text-muted-foreground">
					Preset
				</label>
				<select
					id="provider-preset"
				...
				<p className="text-[10px] text-muted-foreground leading-relaxed">
					{selectedPreset?.description ?? 'Manually configure any compatible model endpoint.'}
				</p>
			</div>
			...
			<div className="flex flex-col gap-1.5">
				...
				<select
					...
					onChange={(e) => {
						markCustom()
						setProvider(e.target.value as ExtConfig['provider'])
					}}
					className="h-8 text-xs rounded-md border border-input bg-background px-2 cursor-pointer"
				>
			...
			<div className="flex flex-col gap-1.5">
				...
				/>
				<p className="text-[10px] text-muted-foreground leading-relaxed">
					OpenAI-compatible uses <code>/chat/completions</code>. Anthropic-compatible uses{' '}
					<code>/v1/messages</code>, which matches cc-switch Claude routing.
				</p>
			</div>
